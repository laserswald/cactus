...
fprint_obj(FILE *f, struct cact_val x)
{
	...
	switch (obj->type) {
    case CACT_OBJ_STRING: return fprintf(f, "%s", ((struct cact_string*)obj)->str);
    case CACT_OBJ_PAIR: return fprint_list(f, x);
...
fprint_sexp(FILE *f, struct cact_val x)
{
    ...
    case CACT_TYPE_SYM:    return fprintf(f, "%s", x.as.symbol->sym);
